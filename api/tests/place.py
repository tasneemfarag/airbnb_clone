...
class AppTestCase(unittest.TestCase):
    ...
    def test_create(self):
        ...
        rv = self.app.post('/places', headers={'Content-Type': 'application/json'}, data=json.dumps(bad_place_13))
        self.assertEqual(rv.status_code, 400)

        ''' Test if owner_id does not exist '''
        rv = self.app.post('/places', headers={'Content-Type': 'application/json'}, data=json.dumps(bad_place_14))
        self.assertEqual(rv.status_code, 404)

        ''' Test if city_id does not exist '''
        rv = self.app.post('/places', headers={'Content-Type': 'application/json'}, data=json.dumps(bad_place_15))
        self.assertEqual(rv.status_code, 404)
    ...
    def test_update(self):
        ...
        ''' Test if place_id does not exist '''
        rv = self.app.put('/places/404', headers={'Content-Type': 'application/json'}, data=json.dumps({'name': 'Super awesomeness!'}))
        self.assertEqual(rv.status_code, 404)

        ''' Test updating protected values '''
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'owner_id': 20}))
        self.assertEqual(rv.status_code, 403)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'city_id': 20}))
        self.assertEqual(rv.status_code, 403)

        ''' Test invalid data type updates '''
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'name': 400}))
        self.assertEqual(rv.status_code, 400)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'description': 400}))
        self.assertEqual(rv.status_code, 400)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'number_rooms': 'nope'}))
        self.assertEqual(rv.status_code, 400)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'number_bathrooms': 'nope'}))
        self.assertEqual(rv.status_code, 400)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'max_guest': 'nope'}))
        self.assertEqual(rv.status_code, 400)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'price_by_night': 'nope'}))
        self.assertEqual(rv.status_code, 400)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'latitude': 'nope'}))
        self.assertEqual(rv.status_code, 400)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'longitude': 'nope'}))
        self.assertEqual(rv.status_code, 400)

        ''' Test valid data type updates '''
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'name': 'This is the next best place after the other'}))
        self.assertEqual(rv.status_code, 200)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'description': 'There is no describing this place'}))
        self.assertEqual(rv.status_code, 200)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'number_rooms': 8}))
        self.assertEqual(rv.status_code, 200)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'number_bathrooms': 5}))
        self.assertEqual(rv.status_code, 200)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'max_guest': 10}))
        self.assertEqual(rv.status_code, 200)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'price_by_night': 125}))
        self.assertEqual(rv.status_code, 200)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'latitude': 32.83838}))
        self.assertEqual(rv.status_code, 200)
        rv = self.app.put('/places/1', headers={'Content-Type': 'application/json'}, data=json.dumps({'longitude': -100.12345}))
        self.assertEqual(rv.status_code, 200)
    ...
    def test_create_by_city(self):
        ...
        ''' Test if latitude is an invalid data type '''
        rv = self.app.post('/states/1/cities/1/places', headers={'Content-Type': 'application/json'}, data=json.dumps(bad_place_12))
        self.assertEqual(rv.status_code, 400)

        ''' Test if longitude is an invalid data type '''
        rv = self.app.post('/states/1/cities/1/places', headers={'Content-Type': 'application/json'}, data=json.dumps(bad_place_13))
        self.assertEqual(rv.status_code, 400)

        ''' Test if owner_id does not exist '''
        rv = self.app.post('/states/1/cities/1/places', headers={'Content-Type': 'application/json'}, data=json.dumps(bad_place_14))
        self.assertEqual(rv.status_code, 404)

        ''' Test if state_id does not exist '''
        rv = self.app.post('/states/404/cities/1/places', headers={'Content-Type': 'application/json'}, data=json.dumps(good_place_by_city_2))
        self.assertEqual(rv.status_code, 404)

        ''' Test if city_id does not exist '''
        rv = self.app.post('/states/1/cities/404/places', headers={'Content-Type': 'application/json'}, data=json.dumps(good_place_by_city_2))
        self.assertEqual(rv.status_code, 404)

        ''' Test if city_id is not linked to state_id '''
        rv = self.app.post('/states/2/cities/1/places', headers={'Content-Type': 'application/json'}, data=json.dumps(good_place_by_city_2))
        self.assertEqual(rv.status_code, 404)
